...
function updateConversationHistory(userParts, modelResponse) {
  // Add user message with potential multi-part content
  conversationHistory.push({
    role: "user",
    parts: Array.isArray(userParts) ? userParts : [{ text: userParts }]
  });
...
function clearHistoryIfExpired() {
  ...
  if (!storedData) return; // If there's no history stored, return
...
async function generateTestSummary(gradingResults) {
  ...
  try {
    ...
    return {
      overallScore: 0,
      strengths: [],
      improvements: ["Error generating summary"],
      recommendations: [],
      feedback: "Could not generate summary due to technical error",
    };
  }
}
